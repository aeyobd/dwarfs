...
let
	...
	plot_xyz!(ax_xyz, orbits, color=COLORS[2], alpha=0.05, time_min=t_min, linestyle=:solid, label="Scl: MW+LMC" => (; alpha=0.5))

	plot_xyz!(ax_xyz, orbits_no, color=COLORS[3], alpha=0.05, time_min=t_min, linestyle=:solid, label="Scl: MW only" => (; alpha=0.5))

	plot_xyz!(ax_xyz, lmc_orbit, time_min=t_min, linestyle=:solid, color=COLORS[1], label="LMC")
	...
	Legend(fig[3, :], ax_xyz[1], tellwidth=false, nbanks=3)
