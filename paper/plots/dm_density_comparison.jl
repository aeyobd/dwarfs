...
function compare_density(gs, modelname_exp; limits=(-0.5, 2.5, -3.5, 1.5), lmc=false, y_low_R_h=nothing, y_low=nothing)
	...
	prof_i, prof_f, _ = ModelUtils.load_stellar_profiles(modelname_exp[1:2]..., "dark_matter")

	plot_prof!(prof_i, color=COLORS[1], linewidth=1.5, linestyle=:dot, label="DM initial")
	plot_prof!(prof_f, color=COLORS[1], linewidth=1.5, linestyle=:solid, label="DM final")
...
@savefig "scl_lmc_density_i_f" let
	...
	ax_scl = compare_density(fig[1,1], modelnames["scl_lmc"], y_low=-6 )
