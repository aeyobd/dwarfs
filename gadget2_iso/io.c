...
void fill_write_buffer(enum iofields blocknr, int *startindex, int pc, int type)
{
  ...
  switch (blocknr)
    ...
    case IO_PERT_ACCEL:
#ifdef OUTPUTPERTURBACCEL
      for(n = 0; n < pc; pindex++)
	if(P[pindex].Type == type)
	  {
	    for(k = 0; k < 3; k++)
	      fp[k] = fac1 * P[pindex].PertAccel[k]; /* perturbing acceletration */
	    fp += 3;
	    n++;
	  }
#endif
...
#ifndef OUTPUTPERTURBACCEL
  if(blocknr == IO_PERT_ACCEL)
    return 0;
#endif
...
void fill_Tab_IO_Labels(void)
{
  ...
  for(i = 0; i < IO_NBLOCKS; i++)
	...
	break;
      case IO_PERT_ACCEL:
	strncpy(Tab_IO_Labels[IO_PERT_ACCEL], "PACC", 4);
	break;
      case IO_DTENTR:
