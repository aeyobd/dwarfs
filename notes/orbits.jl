...
# ╔═╡ 46e72319-a90a-4cb9-a04b-c0ac206c30c3
galaxyname = "tucana4"
...
let
	...
	for i in 1:100
		...
		lines!(1 ./ radii(orbits_ep2020[i]) .^ 2, T, alpha=0.1, color=COLORS[1])
	end
...
let
	...
	for i in 1:1000
		T = max_tidal_force(pot_ep2020, orbits_ep2020[i].positions)
		scatter!(minimum(radii(orbits_ep2020[i])) ^ -2, maximum(T), alpha=1, color=COLORS[1])
	end
...
# ╔═╡ ec6eee51-dfac-4245-ad40-6966bacbc346
plot_peris("ep2020" => orbits_ep2020)
