...
import LinearAlgebra: svd, norm, cross, dot, I

# ╔═╡ 2c194883-a76e-48c1-8cea-76e0ae860045
# Function to fit a great circle to given Cartesian coordinates
function fit_great_circle(coords::Matrix{Float64})
    # Compute the centroid of the coordinates
    centroid = lguys.mean(coords, dims=1)
    # Center the coordinates
    centered_coords = coords .- centroid
    # Perform Singular Value Decomposition
    U, S, V = svd(centered_coords)
...
function rotate_coordinates(coords::Matrix{Float64}, rotation_matrix::Matrix{Float64})
...
norm_vec = fit_great_circle(sky_orbit.ra[idx_orbit], sky_orbit.dec[idx_orbit])
...
rpo, dpo = cartesian_to_spherical(
	rotate_coordinates(
		spherical_to_cartesian(sky_orbit.ra[idx_orbit], sky_orbit.dec[idx_orbit]), create_rotation_matrix(norm_vec)))
