...
cen = lguys.ss_centre(snap, verbose=true)
...
# ╔═╡ 1adedbff-d9f5-4515-9965-594c1112249f
snap_i.masses

# ╔═╡ 32411e1f-da6f-405c-9b6c-20c4a2dbf4a5
rapha = CSV.read("DFDM.csv", DataFrame)

# ╔═╡ 07421f87-88cb-438e-b67c-97f919746c41
begin 
	M = cumsum(snap_i.masses)

	mZero = MeanZero()                   #Zero mean function
	kern = SE(0.0,0.0)
	gp = gp = GP(log10.(radii),log10.(M),mZero,kern) 
end

# ╔═╡ 6a202379-6999-4990-9bb0-6ada240df523
begin 
	log_r= range(-1, stop=3, length=1000)

	log_m, log_m_err = predict_y(gp, log_r)
	log_r = collect(log_r)
...
begin
	...
	Mtot = Mr(r_end)
	Ψs = -lguys.calc_radial_discrete_Φ(M1.(rs_mid) .* diff(rs), rs_mid)
	...
	f(E) =  -1/(√8 * π^2) * (
		quadgk(r -> d2ρ_dΨ2(r) * Ψ1(r) / sqrt(E - Ψ(r)), Ψ_inv(E) * (1+1e-8), r_end)[1]
...
begin 
	...
	plot!(gp, ms=1)
	plot!(log10.(rs), log10.(Mr(rs)))
end

# ╔═╡ a7646485-1dc9-41c0-96d3-5e0a740be11e
begin 
	# this is rapha's method more exactly
	ρ0 = diff(Ms) ./ (4π/3 * diff(rs .^ 3))
	Φ0 = lguys.calc_radial_Φ(rs_mid, diff(Ms))
	Ψ0 = -Φ0.(rs_mid)
...
plot(rs, d2ρ_dΨ2.(rs), xscale=:log10, xlim=(1e-1, 1))

# ╔═╡ df571e5d-2b8c-4476-b459-1c725062ac8e
begin
	plot(legend=false, yscale=:log10, xlabel="ϵ", ylabel="DF")
	plot!(Es, f.(Es))
	plot!(Es,f_2.(Es))
...
begin 
	plot(rs, ρ2.(rs), xscale=:log)
	plot!(rs, lguys.gradient(rs, lguys.gradient(rs, ρ(rs))), label="gradient")
	plot!(ylims=[-0.07, 0.2], ylabel="d2ρ/dr2", xlabel="r")
end
