...
# ╔═╡ 7e213609-c36d-4c8b-9a27-6f674952f5ce
using GLM
...
# ╔═╡ 08a9b448-9955-4516-901c-962b7f0513e6
obs_cen.pmra
...
# ╔═╡ 8800d0a8-77bf-412a-9d7a-9fc1b233064a
let
...
# ╔═╡ 9c6cf7d4-172b-48f5-b69b-9754acfbbccd
PA = 90 - PA_orbit - 180

# ╔═╡ a8a4442e-b180-4ffe-8f91-522e45cac47e
xi_p, eta_p = to_orbit_coords(stars.ra, stars.dec, obs_cen.ra, obs_cen.dec, PA)

# ╔═╡ 22afcc2d-95d1-4999-9b42-0a7c75d054c4
xi_p_orbit, eta_p_orbit = to_orbit_coords(sky_orbit.ra, sky_orbit.dec, obs_cen.ra, obs_cen.dec, PA)
...
sum(filt_leading), sum(stars.probability[filt_leading])

# ╔═╡ 119040e2-ef45-4fc2-809d-aec333e276d0
let 
	fig, ax = ra_dec_axis(r_max / 60)
	...
	scatter!(x[filt_trailing], y[filt_trailing], label="trailing arm", alpha=0.1, markersize=5)
	scatter!(x[filt_leading], y[filt_leading], label="leading arm", alpha=0.1, markersize=5)
...
let
	...
	x = stars.pmra[filt]
	y = stars.pmdec[filt]
	w = weights=stars.probability[filt]
...
let
	...
	dr = 0.1
	limits = (obs_today.pm_ra - dr, obs_today.pm_ra + dr, obs_today.pm_dec - dr, obs_today.pm_dec + dr)
	...
	filt = filt_cen
	hist2d!(stars.pmra[filt], stars.pmdec[filt], weights=stars.probability[filt], limits=limits, colorscale=log10, colorrange=(1e-7, nothing), bins=100)
...
let
	...
	hist2d!(stars.pmra[filt], stars.pmdec[filt], weights=stars.probability[filt], limits=limits, colorscale=log10, colorrange=(1e-10, nothing), bins=100)
...
let
	...
	filt = filt_trailing .& filt_dist
	hist2d!(stars.pmra[filt], stars.pmdec[filt], weights=stars.probability[filt], colorscale=log10, colorrange=(1e-15, nothing), bins=100, limits=limits)
