...
# ╔═╡ 67376f26-af4c-49d9-8eff-c8f283782b37
import DataFrames
...
scl_ell = LilGuys.read_fits(joinpath(data_dir, "jensen+2024_2comp_ell.fits"))

# ╔═╡ b37ad796-1695-4665-b592-b68a99356907
scl_circ = LilGuys.read_fits(joinpath(data_dir, "jensen+2024_2comp_circ.fits"))

# ╔═╡ 39be2272-202a-4bb9-95cb-a9e584589d97
scl_1comp = LilGuys.read_fits(joinpath(data_dir, "jensen+2024_1comp_circ.fits"))

# ╔═╡ 08cf3854-66e9-4358-8925-e069f59f65e6
scl_gaia = LilGuys.read_fits("data/gaia_6deg.fits")
...
# ╔═╡ bd5445b3-8648-4cc0-af59-77fe8595c1ee
DataFrames.dele

# ╔═╡ 9297b743-1eec-4038-b3c0-42c39a5548ce
sum(ismissing.(df.))

# ╔═╡ 62d14e69-2d05-4cb3-8e31-56f4dffef488
...
begin 
	...
	@info sum(ismissing.(df_out.in_qso_candidates))
	
	for col in names(df_out)
		if eltype(df_out[:, col]) <: Union{<:AbstractFloat, Missing}
			df_out[ismissing.(df_out[:, col]), col] .= NaN
		elseif eltype(df_out[:, col]) <: Union{<:String, Missing}
			df_out[ismissing.(df_out[:, col]), col] .= ""
		elseif eltype(df_out[:, col]) <: Union{<:Bool, Missing}
			df_out[ismissing.(df_out[:, col]), col] .= false
		elseif eltype(df_out[:, col]) <: Union{<:Integer, Missing}
			df_out[ismissing.(df_out[:, col]), col] .= -999
		end
...
# ╔═╡ 86306c2e-8c2c-47ea-a833-0f906e1e35dc
sum(ismissing.(df_out.in_qso_candidates))
...
LilGuys.write_fits("processed/j24_umi_all.fits", df_out, verbose=true, overwrite=true)
