...
begin 
	apogee_all = read_fits("processed/apogee_xmatch.fits")
end
...
begin 
	spencer18_all = read_fits("$data_dir/spencer+18_repeated.fits")
...
begin
	pace20 = read_fits("$data_dir/sestito+23_pace_xmatch.fits")
	...
	replace!(pace20.source_id, missing=>-1)
	pace20
end
...
let
	global rv_gaia =  j24[.!ismissing.(j24.dr2_radial_velocity_error), :]
...
let
	...
	println(all_stars[88, :])
	disallowmissing!(all_stars, :ra)
	disallowmissing!(all_stars, :dec)
	all_stars[!, :star_id] .= ""
...
j24[.!ismissing.(j24.dr2_radial_velocity), :]
...
begin 
	...
	memb_filt .&= .!ismissing.(memb_filt)
	memb_filt = disallowmissing(memb_filt)
end
...
icrs = [
	lguys.ICRS(
		...
		distance=82.3 ± 2, pmra=ifelse(ismissing(row.pmra), NaN, row.pmra) ± ifelse(ismissing(row.pmra_error), NaN, row.pmra_error),
		pmdec=ifelse(ismissing(row.pmdec), NaN, row.pmdec) ± ifelse(ismissing(row.pmdec), NaN, row.pmdec_error), 
...
# ╔═╡ d5f80d90-e9f6-473c-acd3-e6c1bffbeb7b
fit_rv_sigma(memb_stars.RV, memb_stars.RV_err)
...
begin 
	pace20_filt = (pace20.PdSph .> 0.2) .=== true
end
...
let 
	...
	filt_good = (pace20_cleaned.RV_std .< 5pace20_cleaned.RV_err  .* sqrt.(pace20_cleaned.RV_count)) .| (pace20_cleaned.RV_count .== 1)
...
apogee_memb_filt = ((apogee.PSAT .> 0.2 ) .& (apogee.RV .> rv_low) .& (apogee.RV .< rv_high) .& (apogee.RV_sigma ./ apogee.RV_err .< 5)) .=== true
...
let 
	...
	filt_good = (spencer_cleaned.RV_std ./ spencer_cleaned.RV_err .< 5 .* sqrt.(spencer_cleaned.RV_count)) .| (spencer_cleaned.RV_count .== 1)
...
gaia_memb_filt = ((rv_gaia.PSAT .> 0.2 ) .& (rv_gaia.RV .> rv_low) .& (rv_gaia.RV .< rv_high)) .=== true
...
# ╔═╡ 56e272a5-31f3-4614-ac31-48bd30b275c9
fit_rv_sigma(memb_stars.RV_p20[.!ismissing.(memb_stars.RV_p20)], memb_stars.RV_err_p20[.!ismissing.(memb_stars.RV_p20)])

# ╔═╡ 932974c6-5001-442c-a8b2-0505b5be5c94
fit_rv_sigma(memb_stars.RV_s18[.!ismissing.(memb_stars.RV_s18)], memb_stars.RV_err_s18[.!ismissing.(memb_stars.RV_s18)])

# ╔═╡ eaf03ca6-4465-4d7f-bd56-8f1ed2b1e7ee
lguys.mean([-244.5, -246.6], [1/0.34, 1/0.59])
...
let
	...
	ps = quantile(rv_meas.PSAT[(rv_meas.PSAT .>= 0) .=== true], LinRange(0, 1, 30))
...
write_fits(joinpath("processed", "old_rv_all.fits"), df_all, overwrite=true)

# ╔═╡ 46b539eb-c195-4ca4-922d-63298afaebbc
write_fits(joinpath("processed", "old_rv_averaged.fits"), df_averaged, overwrite=true)

# ╔═╡ 9028bc37-4d96-42f1-a8bf-459371e45edb
write_fits(joinpath("processed", "old_rv_memb.fits"), df_averaged[memb_filt, :], overwrite=true)
