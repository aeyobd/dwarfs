...
# ╔═╡ 4b281256-c83f-4936-b5b8-8ee990511a4d
hist(θs .% 180)
...
# ╔═╡ f484578a-8492-4c28-9d42-5c6e010925f1
scatter(xi_rot, eta_rot, 
	   axis=(;
			aspect=DataAspect(), 
			xreversed=true))

# ╔═╡ 11f75eec-085c-4222-83c7-41347fe67c39
let
	fig = Figure()
	ax = Axis(fig[1,1], 
			  xlabel = L"$\xi'$ / arcmin",
	...
	for i in 1:600
		m = df_gradient.A[i] * sind(θ_m)  + df_gradient.B[i] * cosd(θ_m)
		x = LinRange(-50, 50, 100)
		lines!(x, m .* x .+ df_gradient.μ[i] .+ Δv_gsr, color=:black, linewidth=1, linestyle=:solid, alpha=0.03)
	end

	errorscatter!(xi_rot, memb_stars.vz .+ Δv_gsr, yerror=memb_stars.vz_err, color=COLORS[2]
				 )
...
# ╔═╡ f48e7fb7-01e3-45be-9df1-779c1153659c
module GaiaUtils
	include("../gaia_utils.jl")
end

# ╔═╡ 89639369-09fc-4999-aa67-ef48d23aa4bd
let
	...
	ax = Axis(fig[1,1], 
			 xreversed=true)
	for i in 1:600
		θ = df_gradient.Θ_grad[i]
		t = [0, 100]
		x = t .* sind(θ)
		y = t .* cosd(θ)
		lines!(x, y, color=:black, linewidth=0.3, linestyle=:solid, alpha=0.05)
	end
...
@savefig "v_gradient_derived" let
	...
	scatter!(60df_gradient.A, 60df_gradient.B, alpha=0.1, markersize=3, 

	   )
	r  = median(df_gradient.r_grad)
	scatter!(r * sind(θ_m), r * cosd(θ_m))

	# scatter!(60df_both.A, 60df_both.B, alpha=0.1, markersize=1, )


	scatter!(0, 0, color=:black, marker=:circle)
...
# ╔═╡ b19afb9d-477a-454c-9486-7fd5c5e04792
vec_orbit = LilGuys.pm2kms.([gsr0.pmra, gsr0.pmdec], gsr0.distance) /(180/π)

# ╔═╡ 3563ac6c-b4c8-4c9c-8481-a644b239f373
let
	fig = Figure()
	ax = Axis(fig[1,1],
			 xreversed = true,
	...
	p = scatter!(memb_stars.xi, memb_stars.eta, color=memb_stars.vz .+ Δv_gsr, colorrange=(rv0 - 2σv .+ Δv_gsr, rv0 + 2σv .+ Δv_gsr), colormap=:bluesreds)


	for i in 1:600
		θ = df_gradient.Θ_grad[i]
		t = [0, df_gradient.r_grad[i] .* 3]
		x = t .* sind(θ)
		y = t .* cosd(θ)
		lines!(x, y, color=:black, linewidth=0.3, linestyle=:solid, alpha=0.05)
	end
	...
	x = t .* sind(θ)
	y = t .* cosd(θ)
	lines!(x, y, color=:black, linewidth=1, linestyle=:solid, alpha=1)

	GaiaUtils.plot_labeled_R_h_ellipse!(obs_properties, 1)

	arrows!([0], [0], [10vec_orbit[1]], [10vec_orbit[2]], color=COLORS[1])

	Colorbar(fig[1, 2], p, label=L"$\text{v}'$ / km/s")
	fig
end
