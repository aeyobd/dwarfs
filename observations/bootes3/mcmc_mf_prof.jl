...
@bind inputs confirm(notebook_inputs(;
		samplename = TextField(40, default="example"),
		mock = CheckBox(),
		simple = CheckBox(),
...
# ╔═╡ db241899-ec51-4bc6-9879-88a849d6f1bb
scatter(stars.xi, stars.eta, markersize=sqrt.(stars.L_sat ./ stars.L_bg) ./ 3, alpha=0.3, color=:black)

# ╔═╡ fb28f68a-4893-4feb-aa29-5cc6acd7d2a8
let
	filt = stars.L_sat ./ stars.L_bg .> 30

	fig = Figure()
	ax = Axis(fig[1,1])
	p = scatter!(stars.xi[filt], stars.eta[filt], color=( stars.gmag[filt]), alpha=0.3)
...
# ╔═╡ 34b52861-19b8-46ba-a7b4-d0f5e543bcb1
prior_tangent = Normal(0, 15)

# ╔═╡ c0427dbf-5261-43a7-8e07-6101938ce324
prior_R_h = LogNormal(log(30), 0.5)

# ╔═╡ 9b421e3d-1759-47e9-9f87-d46dbe28389c
...
let
	samplesout = joinpath(outdir, "samples.$samplename.mcmc_sersic.csv")
	summaryout = joinpath(outdir, "summary.$samplename.mcmc_sersic.csv")
	CSV.write(samplesout, df_sersic)
	CSV.write(summaryout, summary_sersic)
end
...
# ╔═╡ 55d1e2f3-7130-47ef-b8e2-b59b688876ef
let
	fig = Figure()
	ax = Axis(fig[1,1])
	xi, eta = LilGuys.to_tangent(stars.ra, stars.dec, props_ngc5272["ra"], props_ngc5272["dec"])
	...
	prof = LilGuys.SurfaceDensityProfile(dist_ngc5272)

	LilGuys.plot_log_Σ!(ax, prof)
	hlines!(log10(N_stars ./ area_tot))

	prof_model = LilGuys.KingProfile(R_s=0.37, c=10^1.89, k=1)
	@info LilGuys.R_h(prof_model), props_ngc5272["R_h"]

	x = LinRange(0.5, 2, 100)
	y = LilGuys.surface_density.(prof_model, 10 .^ x)
	lines!(x, log10.(y* 3.5*1e4 .+ N_stars ./ area_tot *0.8), color=COLORS[2] )
	fig
end
