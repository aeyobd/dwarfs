...
# ╔═╡ 357d47eb-90a6-4fca-8201-260208599882
all_stars[!, :LLR_S] = @. log10(all_stars.L_S_SAT / all_stars.L_S_BKD) / (log10(all_stars.L_SAT / all_stars.L_BKD))

# ╔═╡ 782aade9-d6f5-4c47-95a8-cdd05198491a
all_stars[!, :LLR_S_norm] = @. log10(all_stars.L_S_SAT / all_stars.L_S_BKD) / (abs(log10(all_stars.L_S_SAT / all_stars.L_S_BKD))
	+ abs(log10(all_stars.L_PM_SAT / all_stars.L_PM_BKD))
	+ abs(log10(all_stars.L_CMD_SAT / all_stars.L_CMD_BKD))
)

# ╔═╡ 01c43679-3577-4810-8eb1-7295141f258f


# ╔═╡ e7f34bb2-7b6f-4921-a69c-1accc0ec56fa
all_stars[!, :LLR_PM_norm] = @. log10(all_stars.L_PM_SAT / all_stars.L_PM_BKD) / (abs(log10(all_stars.L_S_SAT / all_stars.L_S_BKD))
	+ abs(log10(all_stars.L_PM_SAT / all_stars.L_PM_BKD))
	+ abs(log10(all_stars.L_CMD_SAT / all_stars.L_CMD_BKD))
)
...
# ╔═╡ 22e89df5-7b1d-4b5c-af7f-090dfa7dcc25
scatter(all_stars.PSAT[all_stars.F_BEST .== 1], asinh.(all_stars.LLR_S[all_stars.F_BEST .== 1]))

# ╔═╡ b53c0e29-4e10-44a0-a270-5eb25f95051f
scatter(all_stars.PSAT[all_stars.F_BEST .== 1], (all_stars.LLR_S_norm[all_stars.F_BEST .== 1]))

# ╔═╡ 91e931d1-9f9b-40e2-a821-21c57bb6729e
scatter(log10.(all_stars.r_ell[all_stars.F_BEST .== 1]), (all_stars.LLR_S_norm[all_stars.F_BEST .== 1]), alpha=0.1, markersize=3)

# ╔═╡ 3ea8a0ef-cd3e-4e36-8572-de113c1fc73a
scatter(log10.(all_stars.r_ell[all_stars.F_BEST .== 1]), (all_stars.LLR_PM_norm[all_stars.F_BEST .== 1]), alpha=0.1, markersize=3)

# ╔═╡ a76bcff3-14a3-4e42-b4f4-ca4fa9df2760
let 
	dθ = 120
	fig = Figure()
	ax = Axis(fig[1, 1], xlabel=L"\xi / \textrm{arcmin}", ylabel=L"\eta / \textrm{arcmin}", aspect=1, limits=(-dθ, dθ, -dθ, dθ), xgridvisible=false, ygridvisible=false)
