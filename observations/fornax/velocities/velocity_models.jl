...
rv_file = "rv_combined_psat_0.2.fits"
...
let
	...
	RVUtils.plot_samples!(samples, LinRange(0, 100, 100), thin=15)
...
let
	...
	RVUtils.plot_samples!(df_gsr, LinRange(-100, 0, 100), thin=160)
	errorscatter!(midpoints(h.bins), h.values, yerror=h.err, color=COLORS[6])
...
let
	...
	RVUtils.plot_samples!(DataFrame(samples_gradient), LinRange(-100, 0, 100), thin=15)
	errorscatter!(midpoints(h.bins), h.values, yerror=h.err, color=COLORS[6])
