...
samples = DataFrame(sample(RVUtils.model_vel_1c(memb_stars.RV, memb_stars.RV_err, μ_0_prior=Δv_gsr), NUTS(0.65), MCMCThreads(), 10_000, 16))
...
samples_gsr = sample(RVUtils.model_vel_1c(memb_stars.vz, memb_stars.vz_err), NUTS(0.65), MCMCThreads(), 10_000, 16)
...
CSV.write("processed/mcmc_samples_vz.csv", samples_gsr)
...
samples_Rell = sample(model_Rell, NUTS(0.65), MCMCThreads(), 10_000, 16)
...
samples_gradient = sample(model_gradient, NUTS(0.65), MCMCThreads(), 10_000, 16)
...
CSV.write("processed/vz_r_ell_binned.csv", df_r_ell_z)
...
CSV.write("processed/vz_xi_p_binned.csv", df_xi_p)
