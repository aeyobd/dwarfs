...
begin 
	...
	FIGDIR = joinpath(outdir, "figures"); FIGSUFFIX=".mcmc_2plummer"
end
...
@model function double_plummer_model(data::MCMCUtils.GaiaData)
	d_xi = 0# ~ Normal(0, 5)
	d_eta = 0# ~ Normal(0, 5)
	...
	prof = LilGuys.Plummer(r_s=R_s, M=1 - f_outer)
	prof_outer = LilGuys.Plummer(r_s=R_s_outer, M=f_outer)
...
mcmc_model = double_plummer_model(data)
...
let
	...
	for i in 1:100
		...
		prof = LilGuys.Plummer(r_s=df.R_s[i], M=M* (1-df.f_outer[i]))
		prof_outer = LilGuys.Plummer(r_s=df.R_s_outer[i], M=M * df.f_outer[i])
