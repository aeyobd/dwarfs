...
# ╔═╡ ee7829e9-a05a-4632-a712-49da69eaa57f
R_h = TOML.parsefile(joinpath(ENV["DWARFS_ROOT"], "observations/bootes3/observed_properties.toml"))["R_h"]

# ╔═╡ f0dd8813-c4ad-4e66-8c74-0a5c453d580a
function model_label!(text)
	text!(0, 1, text=text, offset=(5, -5), align=(:left, :top), fontsize=10, space=:relative, color=:grey)
	arc!((0, 0), 3*R_h/60, 0, 2π, color=COLORS[3])
end
...
let
	f = scatter(allstars["2x7kpc"].xi_p, allstars["2x7kpc"].eta_p, markersize=1, alpha=0.2,
	   axis=(; aspect=DataAspect(), limits=(-30, 30, -10, 10)))
	hlines!([-1, 1], color=COLORS[2])
...
function plot_stars_hist!(stars; 
						  limits=20 .* 60 .* (-1, 1, -1, 1), bins=200, colorrange=5, kwargs...)
	...
	heatmap!(midpoints(h.xbins) ./ 60, midpoints(h.ybins) ./ 60, y; colorrange=colorrange, kwargs...)
...
function plot_stars_hist(stars; limits=20 .* 60 .* (-1, 1, -1, 1), title="")
	...
	p = plot_stars_hist!(stars; 
						 limits=limits, colormap=:greys, bins=200)
...
# ╔═╡ 279a2146-dec5-4f71-887d-b51f72b26e3b
...
# ╔═╡ 23e5c134-10cf-48f3-ad27-0bd66fcfae54
let
	fig = Figure(size=(6, 5) .* 72)
	ax1 = Axis(fig[1,1], ylabel = L"\eta\ /\ \text{deg}", xreversed=true)

	plot_stars_hist!(allstars["2x7kpc"], colormap=:greys)
	model_label!("2x7kpc")


	ax2 = Axis(fig[1,2], ylabel = L"\eta\ /\ \text{deg}", xlabel = L"\xi\ /\ \text{deg}", xreversed=true)

	plot_stars_hist!(allstars["5x18kpc"], colormap=:greys)
	model_label!("5x18kpc")


	ax3 = Axis(fig[2,1], ylabel = L"\eta\ /\ \text{deg}", xlabel = L"\xi\ /\ \text{deg}", xreversed=true)

	plot_stars_hist!(allstars["3x26kpc"], colormap=:greys)
	model_label!("3x26kpc")

	ax4 = Axis(fig[2,2], ylabel = L"\eta\ /\ \text{deg}", xlabel = L"\xi\ /\ \text{deg}", xreversed=true, xticks=-20:10:10)

	p = plot_stars_hist!(allstars["5x18kpc-P"], colormap=:greys)
	model_label!("5x18kpc-Plummer")
	...
	Colorbar(fig[1:2, 3], p, label=L"\log\, \Sigma_\star\ / \ \text{arcmin}^{-2}")
	...
	@savefig "final_stellar_streams"

	fig
end

# ╔═╡ 85f528f3-f79c-4c51-98f3-c90f55393be3
function along_axis_density_profile(stars, bins = -30:0.1:30, eta_max=1)
	w_scale = Ntot / sum(stars.weights)
