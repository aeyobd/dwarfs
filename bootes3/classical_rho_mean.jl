...
pericentres_boo3 = get_peri("bootes3")

# ╔═╡ 0c1a6ea1-b569-4b50-9901-33ff2d793887
rho_mean_boo3 = get_rho_mean_h("bootes3")
...
function get_MW_mean_density(pot=pot_mw, radii=LinRange(1, 150, 10000); units=Agama.AgamaUnits())
...
let
	...
	ax = Axis(fig[1, 1],
			 ...
			 ylabel = L"$\bar{\rho}_h$ / $\textrm{M}_\odot\,\textrm{kpc}^{-3}$", yscale=log10, 
			  ...
			  yticks=[1e6, 1e7, 1e8],
			 limits =(3, 150, 3e5, 3e8)
			 )


	x, y = get_MW_mean_density()
	lines!(x, 3y, color="black", label=L"3\bar{\rho}_\textrm{MW}")
	text!(15, 2e7, text="likely tidal disruption", fontsize=smallfontsize, rotation=-0.7)
	...
	x = pericentres_boo3
	y = rho_mean_boo3
	errorscatter!([x.middle], [middle(y)], xerror=[error_interval(x)], yerror=[error_interval(y)], color=COLORS[4])
	text!(x.middle, y.middle, text="BooIII", color=COLORS[4])
