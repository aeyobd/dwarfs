...
\newcommand{\sv}{S\textsuperscript{5}}
...
However, \citep{jensen+2026} find that Styx does not appear to be reflected in any overdensities in Blue Horizontal Branch or Red Giant Branch stars, nearly no \Gaia{} stars are consistent with being Styx members, and only a tenous detection in matched filter analysis can be confirmed. A proper dynamical model of \boo{} is needed to understand if a stream like Styx plausably formed from \boo{} and whether \boo{} may reveal something about tidal evolution in the Milky Way. 
...
\begin{table*}
    ...
    \begin{tabular}{l l l l l}
        \hline\hline
        parameter & units & literature & this work & reference \\
        \hline
        $\alpha$ & deg & $209.557 \pm 0.3$ & $209.47 \pm 0.11$ & \citet{li+2026}; \S~\ref{sec:struct_params} \\
        $\delta$ & deg & $26.553 \pm 0.3$ & $26.68 \pm 0.06$ &\citet{li+2026}; \S \ref{sec:struct_params}\\
        DM & mag & $18.43 \pm 0.09$ & --- & \citet{li+2026} \\
        distance & kpc & $48.5 \pm 1.9$ & --- & \citet{li+2026} \\
        $\pmra$ & $\masyr$ & $-1.16 \pm 0.02 \pm 0.017$ & --- &  \citet{battaglia+2022} \\
        $\pmdec$ & $\masyr$ & $-0.88  \pm 0.01 \pm 0.017$ & --- & \citet{battaglia+2022} \\
        $\vlos$ & $\kms$ & 181 & $180$ & \citet{li+2026}; \S 2.3 \\
        $\sigmav$ & $\kms$ & 1.7 & 4 & \citet{li+2026}; \S 2.3 \\
        $R_h$ & arcmin & 33 & $44_{-6}^{+7}$ & \S~\ref{sec:struct_params}\\
        ellipticity & --- & 0.3 & $0.42_{-14}^{+0.11}$ & \S~\ref{sec:struct_params}\\
        position angle & deg & 88 & $89 \pm 9$ & \citet{moskowitz+walker2020}; \S~\ref{sec:struct_params}\\
        $M_V$ & mag & $-5.8\pm0.5$ & $-5.1 \pm 0.24$ & \citet{moskowitz+walker2020}; \S~\ref{sec:luminosity} \\
        $M_\star$ & $\Mo$ & --- & $(1.4 \pm 0.3) \times 10^4$ & \S~\ref{sec:luminosity} \\
...
As discussed in the Introduction, \boo{} has an extrodinary range of measurements for its velocity dispersion, $\sigmav$, ranging from an initial measurment of $\sigmav \sim 14$ down to \citepos{li+2026} recent measurement of $\sigmav \sim 1.69^{+1.03}_{-0.85}\ \kms$. Given that $\sigmav$ is crucial for understanding the tidal evolution of \boo{}, we revisit these measurements here. 

For data, we use the publically available reprocessed Keck/DEIMOS observations \citet{geha+2026} and the \sv{} (acronym) from \citet{li+2026}. 

\citet{geha+2026} reprocess the single Keck/DEIMOS pointing from \citet{carlin+2009} using a homogenised and calibrated anaysis across many archival dwarfs observations. 


\citet{li+2026} instead take recent observations with \sv{} and only report their 21 candidate member stars. 




Fascinatingly, the measurement from \citet{geha+2026} appears to be better constrained. Unfortunately, the \sv{} stars appear to 
...
\begin{acknowledgements}
    We acknowledgeme varios peoples and institutions. Thank you!
...
To search for structure over an extended area in \textit{Gaia}, we download and reprocess \textit{Gaia} sources following \citet{MV2020a}. For simplicity, we select all \textit{Gaia} sources within a rectangle of 10 degrees by 10 degrees in RA and Dec of the literature centre of \boo{} (209.3, 26.8). Next, we apply the quality cuts from \citet{MV2020a, MV2020b}
\begin{itemize}
