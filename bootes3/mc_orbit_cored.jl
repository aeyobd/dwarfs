...
# ╔═╡ 92863e09-9c4e-4887-ac8e-9583fc3dfb21
LilGuys.NFW(r_circ_max=1, v_circ_max=1).M_s
...
samples = let
	...
	N = 10_000
	r_c_rel = 10 .^ rand(Uniform(-1, -0), N)

	vmax = rand(Uniform(20, 45), N) / V2KMS

	lc = abs.(randn(N)) * 0.1
	rmax = [LilGuys.Ludlow.solve_rmax(v, c) for (v, c) in zip(vmax, lc)]

	halos_nfw = [LilGuys.NFW(v_circ_max=vmax[i], r_circ_max=rmax[i]) for i in eachindex(vmax)]
	r_s = [halo.r_s for halo in halos_nfw]
	M_s = [halo.M_s for halo in halos_nfw]

	halos_cored = [LilGuys.CoredNFW(r_s=r_s[i], M_s=M_s[i], r_c=r_s[i] * r_c_rel[i]) for i in eachindex(vmax)]
	...
	df[!, :apo] = orbit_props.apocentre[1:N]
	df[!, :T_orb] = orbit_props.period[1:N]

	df[!, :T_peri] = 2π * df.peri ./ Agama.circular_velocity(pot, df.peri)
	df[!, :t_end] = 10 * rand(N) ./ T2GYR
	df[!, :period] = -orbit_props.period[1:N]
	df[!, :n_peri] = ceil.(df.t_end ./ df.period)

	df[!, :r_circ_max] = LilGuys.r_circ_max.(halos_cored)
	df[!, :v_circ_max] = LilGuys.v_circ.(halos_cored, df.r_circ_max)
	df[!, :T_max] = @. 2π * df.r_circ_max / df.v_circ_max

	df[!, :T_0] = @. sqrt(3π / LilGuys.density(halos_cored, 0))
	
	df[!, :T_geo] = @. sqrt(df.T_0 * df.T_max)

	df[!, :sigma_v_0] = σv_wolf.(halos_cored, der_props_boo3.R_h)
	
	df
end

# ╔═╡ 528c5f46-5009-4755-97aa-c10574dedb33
log10.(0.5)

# ╔═╡ 3c965acc-4ecd-4859-aa7d-3bcb8434deba
hist((samples.T_geo * T2GYR))

# ╔═╡ b9899c7f-c6fe-4904-a014-2017b5eee6a7
hist(log10.(samples.T_geo ./ samples.T_peri), bins=100, 
	axis=(;xlabel = "log10 T_geo / T_peri"))

# ╔═╡ 7186daa6-9028-48b6-9040-9b4374089563
T_dis = Rapha.ecc_factor.(samples.peri, samples.apo)

# ╔═╡ 0cb6a1cd-4905-4dbb-83a2-63c3ea3e2c7b
f_ecc_mean = median(T_dis)

# ╔═╡ 7911b276-9917-4440-b64c-e08cb5bc5ccc
T_orb_mean  = median(samples.period)

# ╔═╡ 256c7ad2-e82e-43e4-b640-e0cbaaad9db6
...
let
	...
	ax = Axis(fig[1,1], 
			...
			yminorticks = [0.01:0.01:0.1; 0.1:0.1:1]
			 )

	
	p = scatter!((samples.peri), (samples.T_geo * T2GYR), markersize=1, color=log10.(samples.r_c_r_s))
	x = LinRange(1, 25, 100)
	t_peri = 2π .* x ./ Agama.circular_velocity.(pot, x)

	filts = [xx*0.9 .< samples.peri .< xx*1.1 for xx in x]
	T_orb = [median(samples.period[filt]) for filt in filts]
	f_ecc = [median(Rapha.ecc_factor.(samples.peri[filt], samples.apo[filt])) for filt in filts]
	y = t_peri * 0.15
	y2 = @. 0.15 * t_peri * (1 + 3 * f_ecc * T_orb ./ (5 ./ T2GYR))
	y3 = @. 0.15 * t_peri * (1 + 3 * f_ecc)
	
	lines!((x), (y * T2GYR), color=COLORS[3])
	band!(ax, x, fill(1e-3, length(x)), y*T2GYR, alpha=0.2, color=COLORS[3])
	band!(ax, x, y*T2GYR, y2*T2GYR,  alpha=0.2, color=COLORS[4])
	band!(ax, x, y2*T2GYR, y3*T2GYR,  alpha=0.2, color=COLORS[9])
	lines!((x), (t_peri * T2GYR), color=:grey)
	xlims!(1, 30)
	ylims!(0.01, 1)
...
# ╔═╡ cc151c4d-4c55-4505-b10d-18fce5e8b2ba
hist(samples.sigma_v_0 * V2KMS)

# ╔═╡ 36fc1c94-dd5b-4b55-a08a-2bc80b6840cd
hist(samples.peri)

# ╔═╡ 7052f954-9d87-4d6a-9956-b5fc2319d41a
Mmx = @. samples.r_circ_max * samples.v_circ_max^2
...
let
	...
	ax = Axis(
		fig[1,1],
		xlabel = "log T_geo / T_peri",  ylabel="log r_c/r_s"
	)
		
	p = scatter!(log10.(samples.T_geo ./ samples.T_peri), log10.(samples.r_c_r_s), 
			markersize=1, color=samples.delta_log_c ./ 0.1,)

	Colorbar(fig[1,2], p, label=L"z-score $\delta\log\, c$")
	vlines!(log10.(0.15))
...
let
	...
	scatter!(samples.r_circ_max, samples.v_circ_max*V2KMS, markersize=1, color=samples.r_c_r_s)
...
filt_samples = samples.T_geo .< 0.15 * samples.T_peri
...
let
	...
	p = scatter!(samples.r_circ_max[filt_samples], samples.v_circ_max[filt_samples]*V2KMS, color=log10.(samples.peri), markersize=1)
