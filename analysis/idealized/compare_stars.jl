...
# ╔═╡ e9a65e2c-ff92-460e-a509-0b8ca3cb91b4
...
model_dir = joinpath(ENV["DWARFS_ROOT"], "analysis", "idealized", inputs.modelname)
...
@bind idx_f confirm(PlutoUI.NumberField(1:length(out.index), default=length(out.index)))

# ╔═╡ 82c4cc0a-3ee7-454e-bb2b-beb55bcb48d3
idx_f
...
let
	...
	for i in eachindex(galaxies)
		...
		errscatter!(prof.log_r, prof.log_Sigma, yerr=prof.log_Sigma_err, label=galaxy, color=COLORS[i+2])
	end
	...
	@savefig "density_i_$(idx_f)_vs_obs" fig
end
