...
# ╔═╡ bbe93c5d-c960-46b4-a1ee-0ff8a7747f72
out.times

# ╔═╡ bc97c47e-7c55-4cc3-9af6-150bddf9d934
md"""
The indexes set by `idx` are the three example orbits to run.
I simply have manually added in the three orbits as the first three entries of this file. The orbits can then be validated with e.g. AGAMA or Galpy.
"""
...
lguys.quantile(peris, [0.02, 0.98])
...
let
	...
	for sym in columns
	    ...
	    stephist!(x, normalization=:pdf, label="MC samples")

	    
	    x_mod = LinRange(μ - 3σ, μ + 3σ, 1000)
	    y_mod = normal_dist.(x_mod, μ, σ)

	    lines!(x_mod, y_mod, label="expected distribution")
