...
# ╔═╡ fae905b2-5d78-44a1-8634-8f6a6f414ce2
using StatsBase: sem
...
sim_dir = ENV["DWARFS_ROOT"] * "/simulations/sculptor/mc_orbits/sys_and_solar/"
...
begin 
	...
	frames_df = lguys.read_fits("$sim_dir/gc_frames.fits")
	@assert size(frames_df, 1) == length(snap)
end

# ╔═╡ 2559bc4c-3bad-4d70-b02f-8fe48b772fb6
special_orbits_dir = ENV["DWARFS_ROOT"] * "/analysis/sculptor/mc_orbits/special_cases"

# ╔═╡ ede94c52-e8ce-4c60-bed7-1a91670b6801
special_sim_dir = ENV["DWARFS_ROOT"] * "/simulations/sculptor/mc_orbits/special_cases"
...
begin 
	...
	frames_df_special = lguys.read_fits("$special_sim_dir/gc_frames.fits")
	@assert size(frames_df_special, 1) == length(snap_special)
end
...
# ╔═╡ b519dc6c-24b5-4d2c-9a57-c711d320c30b
frames_special = [lguys.GalactocentricFrame(
	d=row.d, z_sun=row.z_sun, 
	v_sun=[row.v_sun_x, row.v_sun_y, row.v_sun_z],
	ra=row.ra,
	dec=row.dec
) for row in eachrow(frames_df_special)]
...
function median_residual(observations, frames_df)
	for sym in [:distance, :pmra, :pmdec, :radial_velocity]
		...
		@printf "Δ ln %-15s  = %8.2f \t \n"  sym res
	end

	for sym in [:d, :v_sun_x, :v_sun_y, :v_sun_z, :z_sun]
		md = median(frames_df[:, sym])
		res = (md - μ_frame[sym]) / σ_frame[sym]
		@printf "Δ ln %-15s  = %8.3f \t \n"  sym res
	end
end

# ╔═╡ 26f9ea11-4883-4a2d-b7ee-4f15073960fe
function median_percen(observations, frames_df)
	for sym in [:distance, :pmra, :pmdec, :radial_velocity]
		md = median(getproperty.(observations, sym))
		err = sem(getproperty.(observations, sym))
		@printf "     %-15s  = %8.3f ± %8.3f \n"  sym md err
	end

	for sym in [:d, :v_sun_x, :v_sun_y, :v_sun_z, :z_sun]
		md = median(frames_df[:, sym])
		err = sem(frames_df[:, sym])
		@printf "     %-15s  = %8.3f ± %8.3f\n"  sym md err
	end
end
...
let 
	idx_s = peris .< quantile(peris, p_value*2)

	median_residual(observations[idx_s], frames_df[idx_s, :])
	median_percen(observations[idx_s], frames_df[idx_s, :])
end

# ╔═╡ a16a5fdc-69dd-4ebb-b866-48658b6a2879


# ╔═╡ 34104429-05e0-40a6-83e5-078dbe346504
let
	idx_s = peris .> quantile(peris, 1 - p_value*2)

	median_residual(observations[idx_s], frames_df[idx_s, :])
	median_percen(observations[idx_s], frames_df[idx_s, :])
end

# ╔═╡ 28d6594c-198d-4b77-9444-7346da6a8dd0
let
	idx_s = 1:length(observations)
	median_residual(observations[idx_s], frames_df[idx_s, :])
	median_percen(observations[idx_s], frames_df[idx_s, :])
end
...
for i in 1:length(orbit_labels)
	...
	for name in names(frames_df_special)
...
for i in 1:length(orbit_labels)
	...
	t0 = size(pos, 2) - get_initial_t(rs[i])
	println(t0)
