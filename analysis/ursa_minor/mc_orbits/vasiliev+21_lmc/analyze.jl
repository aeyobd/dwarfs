...
obs_prop_filename = ENV["DWARFS_ROOT"] * "/observations/ursa_minor/observed_properties.toml"
...
peri_qs = lguys.quantile(peris, [p_value, 0.5, 1-p_value])

# ╔═╡ 17a63cc8-84f4-4248-a7b0-c8378454b1f7
idx = [argmin(abs.(peri_qs[2] .- peris)),
	argmin(abs.(peri_qs[1] .- peris)),
	argmin(abs.(peri_qs[3] .- peris)),
]
...
# ╔═╡ 0507e494-b23f-47bf-afaa-944aa5a1d5dd
let
	fig = Figure()
	ax = Axis(fig[1, 1],
...
let
	...
	for sym in columns[1:2]
	    ...
	    p = scatter(x, y, alpha=0.1, markersize=1, color=:black, rasterize=true,
...
# ╔═╡ 3376e202-d4ee-482b-a716-2c98ddbcbd1c
let
	fig = Figure(size=(600, 600))
	plot_kwargs = Dict(
		:color => :black,
	...
	for sym in [:pmra, :pmdec, :distance, :radial_velocity]
		...
		x = [getproperty(o, sym) for o in observations]
		scatter!(x, df_peris_apos.t_last_peri .* lguys.T2GYR; plot_kwargs...)
		for i in eachindex(idx)
	...
	save(joinpath(fig_dir, "t_peri_mc_orbits_corr.pdf"), fig)
	fig
end

# ╔═╡ 75dbbca9-5357-471e-b241-56901122f857
let
	fig = Figure(size=(600, 600))
	plot_kwargs = Dict(
		:color => :black,
	...
	for sym in [:pmra, :pmdec, :distance, :radial_velocity]
		...
		for i in eachindex(idx)
			scatter!(x[idx[i]], apos[idx[i]]; orbit_points_kwargs[i]...)
		end
	end
...
let
	...
	for sym in [:pmra, :pmdec, :distance, :radial_velocity]
		...
		μs = getproperty(obs, sym)
		σs = getproperty(err, sym)
		
		x_mod = LinRange(minimum(x), maximum(x), 1000)

		y_mod = normal_dist.(x_mod, μs, σs)
		lines!(x_mod, y_mod, label="observations")
...
# ╔═╡ 9f6a5c54-25fe-4677-a900-477ee0894e95
...
# ╔═╡ f71d31fb-31a6-4773-ad98-2b8256526f16
md"""
#  Median shifts...
"""
