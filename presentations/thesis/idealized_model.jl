...
# ╔═╡ 2a09eb3f-b56f-4c0d-906e-a88d44c80cc0
CairoMakie.activate!(type=:png)

# ╔═╡ 4c1ab53f-69b4-437c-9713-dc22bfce0db3
module ModelUtils
	include("../../thesis/plots/model_utils.jl")
end
...
# ╔═╡ 18ae7c88-c8cd-4892-bc3b-f49ebd84684c
import StatsBase: sample, weights

# ╔═╡ 8edc5ea5-63f7-4483-88eb-40445a575255
stars_sampled = stars_f[sample(1:size(stars_f, 1), weights(stars_f.weights), 20000), :]

# ╔═╡ 01ba0454-b5c4-43a5-b246-cc361c4079b8
x_sun = [-8.122, 0, 0]

# ╔═╡ fe44a39d-3a1a-4664-a483-6e0ee646868d
LilGuys.transform(LilGuys.Galactocentric, ICRS(ra=0., dec=0., distance=0.))

# ╔═╡ 77b629cd-b698-462a-931a-18541609bbde
scale_theme_element!(:linewidth, 2)

# ╔═╡ e8421d4d-39d3-4c11-ada4-c03c386b53b7
theme(:size)

# ╔═╡ 928c34c7-22bb-4cd5-84c1-987e2cddac39
function plot_sun!()
	scatter!(x_sun[1], x_sun[2], color=COLORS[9], marker=:star5, strokewidth=theme(:linewidth)[]/2, strokecolor=:black, markersize=1.5*theme(:markersize)[])

end

# ╔═╡ b1694319-1f17-4179-87f1-226e30eef9ff
function spiral!(num_arms=5)
	t = LinRange(0, 1π, 100)
	A = 6

	for i in 1:num_arms
		θ_0 = (i-1) / num_arms * 2π
		lines!(A *t .* cos.(t .+ θ_0), A *t .* sin.(t .+ θ_0), color=COLORS[1])
	end
	
end

# ╔═╡ d99d8a90-055d-4130-9822-2cdc1a136de6
x_cen_transformed = get_vectors(snap_f) * snap_f.x_cen # should be positive x only

# ╔═╡ a362170b-d0d2-4159-a909-b41fded53181
"""
    plot_tidal_arrows!(ax; L=1.0, gap=0.3, strength=0.5, kwargs...)

Plot 4 arrows representing tidal forces around a body centered at (0,0).

- L        : characteristic body size (radius-like)
- gap      : distance from the surface where arrows are placed
- strength : arrow length scaling
"""
function plot_tidal_arrows!(
    ax;
    gap = 0.3,
    strength = 0.5,
	x0 = x_cen_transformed[1],
	y0 = x_cen_transformed[2],
	strength_ratio = 1.379,
    kwargs...
)
    ...
    positions = Point2f[
        (x0 + r,  y0),   # +x (radial stretch)
        (x0 - r,  y0),   # -x (radial stretch)
        (  x0, y0 + r),   # +y (tangential compression)
        (  x0, y0 - r)    # -y (tangential compression)
    ]

    # Tidal force directions
    directions = Vec2f[
        ( strength_ratio*strength,  0),   # stretch outward
        (-strength_ratio*strength,  0),   # stretch outward
        ( 0, -strength),   # compress inward
        ( 0,  strength)    # compress inward
    ]
    ...
    arrows2d!(
        ...
        positions,
        directions;
		shaftwidth= theme(:linewidth),
		tiplength = 2*theme(:markersize)[],
		tipwidth = 2*theme(:markersize)[] * 2/sqrt(3),
        kwargs...
    )
...
# ╔═╡ 723a3da2-3cd4-4fa3-aa8d-458eee803e3e
@savefig "tides_diagram" let
	fig = Figure(size=(1728, 800))
	ax = Axis(fig[1,1], autolimitaspect=1)
	hidespines!()

	scatter!(stars_sampled.x, stars_sampled.y, color=:black, markersize=theme(:markersize)[]/2)
	...
	# arrows2d!(x_cen_transformed[1:1], x_cen_transformed[2:2], orbit_direction[1:1], orbit_direction[2:2], lengthscale=10, align=-1.2, color=COLORS[3])


	plot_tidal_arrows!(ax, gap=10, strength=5, color=COLORS[1])
	
	# lines!([30, 50], [-20, -20], color=:black)
	# text!(30, -20, text="20 kpc")
...
# ╔═╡ b8c18653-345c-4211-98d1-1bd255f84a96
snap_f.x_cen

# ╔═╡ 771d1d46-9b63-4083-90b7-898b500a4da5
1066.6666666666667/2

# ╔═╡ 36360700-c63a-4e7a-b1ac-6a8ea8b899b6
theme(:size)

# ╔═╡ 3b80f30d-69db-4591-ba6f-8af6c0c4af7d
stars_f
...
function plot_stars_2d(gs, stars; R_h=NaN, R_h_label=false, 
	...
	h = histogram2d(stars.x, stars.y, bins, weights=stars.weights * 10^norm, normalization=:density)
...
let
	...
	p = plot_stars_2d(fig[1,2], stars_f, r_max=r_max, orbit_direction=orbit_direction)
...
function plot_density_i_f!(ax, profiles)
	for (i, prof) in enumerate(profiles)
		lines!(ax, LilGuys.log_radii(prof), LilGuys.log_surface_density(prof),  color=COLORS[1], linestyle=[:dot, :solid][i], label = labels[i], linewidth=theme(:linewidth)[]/2)

	end
end

# ╔═╡ a6369d2f-1b8a-433c-8419-cbecb0f7d521
@savefig "density_profile_tides_example" let
	fig = Figure(size=(800, 668))
	r_max = 10
	...
	axislegend(position=:lb, margin=theme(:Legend)[:margin])
