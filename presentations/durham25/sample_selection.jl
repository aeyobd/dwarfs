...
# ╔═╡ 50e90e5e-8b0e-46a8-acad-b0e157db0aed
all_no_missing = all_stars[.!ismissing.(all_stars.xi) .& .!ismissing.(all_stars.eta) .& .!ismissing.(all_stars.pmra) .& .!ismissing.(all_stars.bp_rp), :]
...
styles = Dict(
	:all => (;	markersize=3, color=grey, 
		label="all" => (;markersize=12, alpha=0.3),
		rasterize=10,
		alpha=0.1
	),
	:best => (;	markersize=3, color=grey, 
		label="quality" => (;markersize=12, alpha=0.3),
		rasterize=10,
		alpha=0.1
	),
	:members => (;
...
# ╔═╡ c44a41be-79a3-4e6f-a1a1-d2037a240439
@savefig "$(galaxyname)_all_selection" let
	fig = Utils.compare_j24_samples(
		Dict(:all => all_no_missing), styles,
		observed_properties,
		Gmin=15.9,
		references=false
	)
...
# ╔═╡ f8f4c422-55a9-421a-93bb-d0b2f1475da1
@savefig "$(galaxyname)_best_selection_noref" let
	fig = Utils.compare_j24_samples(
