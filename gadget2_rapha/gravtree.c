...
void gravity_tree(void)
{
...
#ifdef EXTERNAL_POTENTIAL /* add an external source of gravitational force */

  if(ThisTask == 0)
    printf("adding external potential force contribution.\n");
  ...
  for(i = 0; i < NumPart; i++)
	...
	{
	  pertaccel = f_ext(i, j);
	  P[i].GravAccel[j] += pertaccel;
	  P[i].PertAccel[j] = pertaccel;
	}

  if(ThisTask == 0)
    printf("external contribution done.\n");
