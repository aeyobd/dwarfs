...
#define MAXTAGS 300
  ...
  if(ThisTask == 0)		/* read parameter file on process 0 */
    {
      ...
      strcpy(tag[nt], "AnchorFile");
      addr[nt] = All.anchor_file;
      id[nt++] = STRING;
      ...
      strcpy(tag[nt], "InterpTableNFiles");
      addr[nt] = &All.pert_interp_nfiles;
      id[nt++] = INT;
	
      strcpy(tag[nt], "InterpTableFileBase");
      addr[nt] = &All.pert_interp_fbase;
      id[nt++] = STRING;

#elif PERT_PROFILE == ISOTHERMAL
      strcpy(tag[nt], "MExt");
      addr[nt] = &All.pert_M;
      id[nt++] = DOUBLE;
...
#elif PERT_PROFILE == MIYAMOTONAGAI			/* begin James Lane added 04/27 */
      strcpy(tag[nt], "MExt");
      addr[nt] = &All.pert_M;
      id[nt++] = DOUBLE;
      ...
      strcpy(tag[nt], "bExt");
      addr[nt] = &All.pert_b;
      id[nt++] = DOUBLE;				/* end James Lane added 04/27 */

#elif PERT_PROFILE == MILKYWAY			/* begin James Lane added 05/19 */
      strcpy(tag[nt], "MdExt");
      addr[nt] = &All.pert_Md;
      id[nt++] = DOUBLE;
      ...
      strcpy(tag[nt], "dExt");
      addr[nt] = &All.pert_d;
      id[nt++] = DOUBLE;				/* end James Lane added 05/19 */

#elif PERT_PROFILE == MWPOTENTIAL2014
	strcpy(tag[nt], "MdExt");
	addr[nt] = &All.pert_Md;
	id[nt++] = DOUBLE;
        ...
        strcpy(tag[nt], "MhvirExt");
        addr[nt] = &All.pert_Mhvir;
        id[nt++] = DOUBLE;
        ...
        strcpy(tag[nt], "alphaExt");
        addr[nt] = &All.pert_alpha;
        id[nt++] = DOUBLE;

	strcpy(tag[nt], "concExt");
        addr[nt] = &All.pert_conc;
        id[nt++] = DOUBLE;
        

#elif PERT_PROFILE == RAPHAMW
        strcpy(tag[nt], "MdthinExt");
        addr[nt] = &All.pert_Md_thin;
        id[nt++] = DOUBLE;
        
        strcpy(tag[nt], "MdthickExt");
        addr[nt] = &All.pert_Md_thick;
        id[nt++] = DOUBLE;
        ...
        strcpy(tag[nt], "athinExt");
        addr[nt] = &All.pert_a_thin;
        id[nt++] = DOUBLE;
        
        strcpy(tag[nt], "athickExt");
        addr[nt] = &All.pert_a_thick;
        id[nt++] = DOUBLE;
        
        strcpy(tag[nt], "abulgeExt");
        addr[nt] = &All.pert_abulge;
        id[nt++] = DOUBLE;
        
        strcpy(tag[nt], "bthinExt");
        addr[nt] = &All.pert_b_thin;
        id[nt++] = DOUBLE;
        
        strcpy(tag[nt], "bthickExt");
        addr[nt] = &All.pert_b_thick;
        id[nt++] = DOUBLE;
  ...
  if(ThisTask == 0)
    {
      read_anchorfile(All.anchor_file);
      All.TimeMax = (double) All.t_anchors[All.n_anchors-1];
    }
...
#ifdef PERT_PROFILE
#if PERT_PROFILE == CUSTOMTABLE

  if(ThisTask == 0)
    {
      read_interpfile(All.pert_interp_fbase, All.pert_interp_nfiles);
    }
...
/*! This function reads a table with a list of external potential amplitude anchors. 
 *  The first line should specify the number of anchors.
 *  Subsequent lines should be pairs of simulation times and relative amplitudes.
 */
#ifdef EXTERNAL_POTENTIAL
int read_anchorfile(char *fname)
{
  FILE *fd;

  if(!(fd = fopen(fname, "r")))
    {
      printf("can't read anchor file '%s'\n", fname);
      return 1;
    }

  fscanf(fd, "%i", &All.n_anchors);

  unsigned int i = 0;
  for(i = 0; (i < All.n_anchors) && (i < MAX_ANCHORS); i++)
    {
      fscanf(fd, "%lg %lg", &All.t_anchors[i], &All.amp_anchors[i]);
    }
...
/*! This function reads a set of files with times and corresponding
 *  cumulative mass profiles for a perturbing potential. The first
 *  line of each file should be a time, subsequent lines should be 
 *  radius, enclosed mass pairs. The number of files and the base
 *  filename are read from the parameterfile.
 */

#ifdef EXTERNAL_POTENTIAL
#ifdef PERT_PROFILE
#if PERT_PROFILE == 4
int read_interpfile(char *fbase, int nfiles)
{
  FILE *fd;
  ...
  for(i = 0; (i < nfiles); i++)
    {
      ...
      strcat(fname, "_");
      char numstring[10];
      sprintf(numstring, "%.3i",i);
      strcat(fname, numstring);
      if(!(fd = fopen(fname, "r")))
	{
	  printf("can't read custom interpolation table file '%s'\n",fname);
	  return 1;
	}

      fscanf(fd, "%lg", &All.pert_interp_t[i]);

      unsigned int j;
      for(j = 0; (j < MAX_INTERPTABLE); j++)
	{
	  if(fscanf(fd, "%lg %lg", &All.pert_interp_r[j], &All.pert_interp_M[i][j]) == EOF)
	    {
	      for(; (j < MAX_INTERPTABLE); j++)
		{
		  All.pert_interp_r[j] = -1;
		  All.pert_interp_M[i][j] = -1;
		}
	      break;
	    }
	}
  ...
  for(; (i < MAX_INTERPTABLE); i++)
    {
      All.pert_interp_t[i] = -1;
      unsigned int j;
      for(j = 0; (j < MAX_INTERPTABLE); j++)
	{
	  All.pert_interp_M[i][j] = -1;
