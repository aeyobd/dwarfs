...
orbits_m_lmc = orbits .- Ref(lmc_orbit)
...
tides_lmc = OrbitUtils.scalar_tidal_forces.(pot_lmc, positions, agama_units=agama_units)

# ╔═╡ 0e2e3c40-7aca-4fd0-b0c8-0850a4e1c8b5
max_tides = maximum.(tides)

# ╔═╡ 9976635c-51e9-48ab-87e6-49662aeb58a2
lguys.plot_xyz(orbits_m_lmc[argmax(max_tides)].positions)

# ╔═╡ 809664c4-9a85-4f87-ab9e-eec4178f6f80
scatter(log10.(max_tides), df_props.pericentre[idx])

# ╔═╡ 9adb284c-5f1e-4696-9a39-bffc86124c54


# ╔═╡ 6cc4d76f-cc3c-4619-8dbe-4dea2aed0a10
scatter(log10.(max_tides), df_props.pericentre_lmc[idx])
...
let
	...
	for i in eachindex(orbits)
		lines!(rs[i], log10.(abs.(tides[i])), color=COLORS[1], alpha=0.1)
	end
...
let
	...
	for i in eachindex(orbits)
		lines!(radii(orbits_m_lmc[i]), log10.(abs.(tides[i])), color=COLORS[1], alpha=0.1)
	end
...
let
	...
	ax = Axis(fig[1,1], 
			 xlabel = "time / Gyr",
			 ylabel = "log10 tidal strain")




	for i in idx
		lines!(times * T2GYR, log10.(abs.(tides_no[i])), color=COLORS[2], alpha=0.1)
	end
	for i in idx
		lines!(times * T2GYR, log10.(abs.(tides_lmc[i])), color=COLORS[3], alpha=0.1)
	end
	for i in idx
		lines!(times * T2GYR, log10.(abs.(tides[i])), color=COLORS[1], alpha=0.1)
	end

	lines!([NaN], [NaN], color=COLORS[1], alpha=0.5, label="combined tides")
	lines!([NaN], [NaN], color=COLORS[2], alpha=0.5, label="MW tides")
	lines!([NaN], [NaN], color=COLORS[3], alpha=0.5, label="LMC tides")
