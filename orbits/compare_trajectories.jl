...
# ╔═╡ 0884d4f8-3858-4228-919b-4274b6367010
@bind modelname_alt TextField(24, default="example") |> confirm
...
# ╔═╡ f0168a3c-42ab-431b-88c0-52dcf688780f
idx, orbits = read_orbits(galaxyname, modelname)

# ╔═╡ 7cfec8fe-2b5a-4939-a04c-a0cdcc0292ca
icrs0 = LilGuys.coords_from_df(df_props[idx, :])

# ╔═╡ 8ce0835c-b16c-4802-b269-38253aaab609
idx2, orbits2 = read_orbits(galaxyname, modelname_alt)

# ╔═╡ a15cc452-f64e-4b47-972e-e84614c2d04f
_, orbits3 = read_orbits(galaxyname, "vasiliev+21_noreflex")
...
# ╔═╡ 39d647d8-f549-41b8-971a-324503a67b96
positions2 = LilGuys.positions.(orbits2)
...
# ╔═╡ f1e65852-08bc-4ea3-85e3-2c782fdf67d4
function plot_r_t!(orbits, color=COLORS[1]; kwargs...)
	rs = lguys.radii.(lguys.positions.(orbits))
	
	for i in eachindex(orbits)
		lines!(orbits[i].times * lguys.T2GYR, rs[i], alpha=0.1, color=color; kwargs...)
	end
end
...
function plot_r_t(orbits; kwargs...)
	...
	plot_r_t!(orbits; kwargs...)

	fig
end

# ╔═╡ 068294d3-7eb8-4a9d-b52c-4412135a8f06
plot_r_t(orbits)

# ╔═╡ 1db5bf38-ba0f-48cc-93fb-480839ca74d4
let
	fig = plot_r_t(orbits, label="MW only")
	plot_r_t!(orbits2, color=COLORS[2], label="MW+LMC+reflex")
	plot_r_t!(orbits3, color=COLORS[4], label="MW+LMC")
...
let
	...
	for i in eachindex(positions)
		...
		lines!(R, z, 
			alpha=0.1, color=COLORS[1], linestyle=:solid,
		)
	end
