...
function v_rad_hist(snap, bins=100)
	...
	v_rad = LilGuys.calc_v_rad(snap)
	logr = log10.(calc_r(snap))
	x_bins, v_bins, err = LilGuys.histogram(logr, bins, weights=v_rad .* mass)
	_, counts, _ = LilGuys.histogram(logr, x_bins, weights=mass)
...
function get_r_b(idx)
	return LilGuys.calc_break_radius(LilGuys.calc_σv_1d(out[idx]), get_delta_t(idx))
end

# ╔═╡ 37bf0c8d-9ef3-4cf8-b818-ff37f2d856eb
profiles = [LilGuys.StellarProfile(out[i]) for i in idxs]
...
@savefig "idealized_break_radius" let
	...
	for (i, prof) in enumerate(profiles)
		lines!(prof.log_r, prof.log_Sigma, color=i, colorrange=(1, length(idxs)))


		x = log10(r_bs[i])
		y = LilGuys.lerp(prof.log_r, prof.log_Sigma)(x)
		dy = 0.5
		h = 0.5
		t = round(times[i] * T2GYR * 1e3, digits=0)
		arrows!([x], [y+h+dy], [0], [-h],  color=i, colorrange=(1, length(idxs)),
		arrowsize=5, label = L"$\Delta t = %$(t)$ Myr")
	...
	ax_v = Axis(fig[2,1],
		...
		limits=((-1.5, 1.0), (-10, 20))
	)

	for i in eachindex(idxs)
		snap = out[idxs[i]]
		
		x, y, ye = v_rad_hist(snap, 150)
		lines!(midpoints(x), y * V2KMS, color=i, colorrange=(1, length(idxs)), linewidth=1)
		arrows!([log10(r_bs[i])], [-5], [0], [3],  color=i, colorrange=(1, length(idxs)), linewidth=1, arrowsize=5)
	end
