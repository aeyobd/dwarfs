...
@savefig "v_circ_potential" let
	...
	plot_v_circ!(Φ_ep._py, label="total", linestyle=:solid, color=:black, linewidth=3)

	plot_v_circ!(Φ_bulge, label="bulge", linestyle=:dot, color=COLORS[1], linewidth=2)
	plot_v_circ!(Φ_thin + Φ_thick, label = "disk", linestyle=:dashdot,  color=COLORS[2], linewidth=2)
	plot_v_circ!(Φ_nfw, label="halo", linestyle=:dash,  color=COLORS[3], linewidth=2)
