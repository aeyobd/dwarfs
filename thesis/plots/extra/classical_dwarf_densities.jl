...
let
    ...
    x = LinRange(-1.5, 1.2, 1000)
    y = log10.(LilGuys.surface_density.(LilGuys.Sersic(n=1), exp10.(x)))
    lines!(x, y, color=:black, label="2D exponential", linewidth=0.8*theme(:linewidth)[])
    

	for (galaxy, kwargs) in galaxies_scatter
		prof = profiles[galaxy]
			errorscatter!(ax, prof.log_R, prof.log_Sigma, yerror=LilGuys.error_interval.(prof.log_Sigma), markersize=theme(:markersize)[]*0.8; kwargs...)
	end
