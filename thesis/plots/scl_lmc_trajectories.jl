...
best_orbit = LilGuys.Orbit(joinpath(modeldir_best, "orbit_smallperilmc.csv"))
...
# ╔═╡ 376adb0a-9da2-4272-be52-39d851669761
theme(:size)

# ╔═╡ 0e864c8d-eb7c-4038-bc0f-2d94735de21e
orbits[1].times
...
let
	...
	plot_xyz!(ax_xyz, lmc_orbit, time_min=t_min, linestyle=:solid, color=COLORS[3])
	...
	# labels
	lines!([NaN], [NaN], color=COLORS[2], alpha=0.5, label="Scl MW only")
	lines!([NaN], [NaN], color=COLORS[1], alpha=0.5, label="Scl MW+LMC")
	lines!([NaN], [NaN], color=COLORS[3], label="LMC")
	...
	plot_rt!(ax_rt2, orbits .- [pos_lmc_resampled], color=COLORS[1], alpha=0.05, linestyle=:solid)
	xlims!(-9, 0.2)
