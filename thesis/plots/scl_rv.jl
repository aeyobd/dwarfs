...
stars = LilGuys.read_fits(ENV["DWARFS_ROOT"] * "/observations/sculptor/processed/sculptor_memb_rv.fits")
...
@savefig "scl_v_z_obs_scatter" let
	fig = Figure()
	ax = Axis(fig[1,1],
		xlabel = L"$\xi$ / arcmin",
		ylabel = L"$\eta$ / arcmin",
	...
	p = voronoiplot!(stars.xi, stars.eta, color = stars.vz, colormap=:redsblues, markersize=2, strokewidth = 0, colorrange=colorrange, show_generators=false)
	
	scatter!(stars.xi, stars.eta, color = :black, markersize=1, alpha=0.2, strokewidth=0)
