...
stellar properties. We calculate the velocity dispersion using all
particles within \(1\,\kpc\) of the dwarf's centre. The stellar velocity
dispersion decreases by only \(\sim1\,\kms\) and the half-light radius
expands by \(\sim 10\%\). This is consistent with adiabatic expansion
due to the reduction of the total mass \citep[e.g.,][]{stucker+2023}.
Altogether, Galactic tides negligibly impact Scl's stellar component.
...
the corresponding colour. The \texttt{L2M11} has a lighter LMC mass, and
the \texttt{L3M10} model has a lighter MW mass than our fiducial
\texttt{L3M11} LMC model.}\label{fig:scl_orbit_lmc_uncert}
\end{figure}
